...
IMPORT $.Types.KMeans_Model AS KTypes;
IMPORT $.Types.KMeans_Model.Ind1 as KM1;

/**
  * Classic KMeans Clustering.
  *
  * <p>Clustering Algorithms are a branch of unsupervised machine learning algorithms.
  * They automatically categorize observations(points) into groups without pre-defined
  * labels. KMeans[1] is one of the most well-known clustering algorithms. Given the data
  * points for clustering and the K initial centroids of each cluster, the KMeans algorithm
  * can automatically group each sample into one cluster.
  *
  * <p>KMeans is a popular clustering method for cluster analysis in data mining.
...
EXPORT KMeans(INTEGER max_iter = 100 , REAL t = 0.00001) := MODULE
  ...
  /**
    ...
    *                    'number').
    * @param y           The centers for distance calculation in DATASET(NumericField) format.
    *                    Each observation (e.g. record) is identified by
    *                    'id', and each feature is identified by field number (i.e.
    *                    'number').
    * @param mode        A discrete value 1 or 0 to define how to calculate the Euclean
    *                    distances between x and y in each model identified by wi.
    *                    Mode 1: calculate the distances of each data pair of x and y.
    *                    Mode 2: calculate the distances of data pairs with same id in x and y.
  ...
  END;
  /**
    * Function getClosest() is used to calculate the closest center of sample
    * via Euclidean distance.
    * @parm x            The samples for distance calculation in DATASET(NumericField) format.
  ...
  END;
  /**
    * Extract the number of iterations that each work item took to converge,
